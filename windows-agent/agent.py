...
# Errors that mean "don't retry, just mark manual_required immediately"
SKIP_ERRORS = [
...
def is_skip_error(error: str) -> bool:
    """Check if this error means we should skip retries and go straight to manual."""
    if not error:
        return False
...
def main():
    ...
    with sync_playwright() as p:
        ...
        # Launch with all anti-detection args
        context = p.chromium.launch_persistent_context(
            ...
            ignore_default_args=["--enable-automation"],  # Remove the flag that causes the banner
        )

        # Apply stealth to the CONTEXT so all pages (including new tabs) get patched
        STEALTH.apply_stealth_sync(context)

        # Also apply to any new pages/tabs that open
        def on_new_page(new_page):
            STEALTH.apply_stealth_sync(new_page)
            log.info("Stealth applied to new page/tab")
            ...
            try:
                ...
                for job in jobs:
                    ...
                    for attempt in range(retry_config["max_attempts"]):
                        ...
                        # Check if this error means "don't bother retrying"
                        error_msg = result.get("error", "unknown")
                        if is_skip_error(error_msg):
                            log.warning(f"Non-retryable error: {error_msg} — skipping to manual")
                            break
